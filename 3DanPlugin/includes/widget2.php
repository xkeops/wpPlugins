...
switch ($dp3_type) {
	...
	$pPluginContent = plugins_url( 'res/Android_1.txt', dirname( __FILE__ ) );		
        break;
    case 'tShirt':
	$pPluginContent = plugins_url( 'res/tShirt.txt', dirname( __FILE__ ) );		
        break;
    default:
	$setType = "black";
	$pPluginContent = plugins_url( 'includes/iphone_'.$setType.'.txt', dirname( __FILE__ ) );		
    ...
    <label for="<?php echo $this->get_field_id('type'); ?>">
        <?php _e('tShirt:'); ?>
        <input class="" id="<?php echo $this->get_field_id('tShirt'); ?>" name="<?php echo $this->get_field_name('type'); ?>" type="radio" value="tShirt" <?php if($type === 'tShirt'){ echo 'checked="checked"'; } ?> />
    </label>
		</p>
		<?php 
